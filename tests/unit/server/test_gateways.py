...
class TestDatabase(unittest.TestCase):
    ...
    def test_when_name_not_reserved_then_return_empty_list(self):
        self.assertFalse(self.uut.get_active_reservations(name=NAME))

    def test_when_reservation_success_then_return_reserved_list(self):
        self.assertTrue(self.uut.reserve(name=NAME,
                                         user_name=USER_NAME))
    ...
    def test_when_stuff_already_reserved_then_return_false(self):
        self.assertTrue(self.uut.reserve(name=NAME, user_name=USER_NAME))

        self.assertFalse(self.uut.reserve(name=NAME,
                                          user_name=USER_NAME))

    def test_when_release_failed_then_return_false(self):
        self.assertFalse(self.uut.release(name=NAME))

    def test_should_release_reservation(self):
        self.uut.reserve(name=NAME, user_name=USER_NAME)

        self.assertTrue(self.uut.release(name=NAME))
        self.assertFalse(self.uut.get_active_reservations())
